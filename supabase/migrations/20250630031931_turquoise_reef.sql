/*
  # Create appointment calendar infrastructure

  1. New Tables
    - `availability_settings`
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to users)
      - `working_hours` (jsonb) - Regular working hours
      - `time_blocks` (jsonb) - Special time blocks (vacations, breaks, etc.)
    ...
    - `appointment_series`
      ...
      - `interval` (integer) - repeat every X days/weeks/months
      - `end_date` (timestamptz) - when the series ends
      - `occurrences` (integer) - number of occurrences
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)

  2. Modify appointments table
    - Add `series_id` (uuid, foreign key to appointment_series)
  ...
  3. Security
    ...
    - Add policies for appointments with series
*/
...
BEGIN
  ...
  appointment_start := NEW.appointment_date::time;
  appointment_end := (NEW.appointment_date + (NEW.duration_minutes || ' minutes')::interval)::time;
  ...
  IF appointment_start < working_start OR appointment_end > working_end THEN
  ...
  IF block_count > 0 THEN
    ...
    AND (
      ...
      AND (
        (appointment_start >= (jsonb_array_elements(time_blocks)->>'startTime')::time 
         AND appointment_start < (jsonb_array_elements(time_blocks)->>'endTime')::time)
        OR
        (appointment_end > (jsonb_array_elements(time_blocks)->>'startTime')::time 
         AND appointment_end <= (jsonb_array_elements(time_blocks)->>'endTime')::time)
        OR
        (appointment_start <= (jsonb_array_elements(time_blocks)->>'startTime')::time 
         AND appointment_end >= (jsonb_array_elements(time_blocks)->>'endTime')::time)
      )
...
BEGIN
  ...
  current_date := NEW.appointment_date;
  current_occurrence := 1;
  
  -- Determine end condition
  IF NEW.recurrence_rule->>'endDate' IS NOT NULL THEN
    end_date := (NEW.recurrence_rule->>'endDate')::timestamptz;
    occurrences := NULL;
  ELSIF NEW.recurrence_rule->>'occurrences' IS NOT NULL THEN
    end_date := NULL;
    occurrences := (NEW.recurrence_rule->>'occurrences')::integer;
  ELSE
  ...
  WHILE (end_date IS NULL OR current_date < end_date) AND (occurrences IS NULL OR current_occurrence < occurrences) LOOP
