...
-- Notifications policies - check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Notification preferences policies - check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'on_user_created_notification_preferences'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'prescription_notification_trigger' AND 
          tgrelid = 'prescriptions'::regclass::oid
  ) THEN
...
EXCEPTION
  WHEN undefined_table THEN
    -- If prescriptions table doesn't exist yet, we'll skip this trigger
    RAISE NOTICE 'Prescriptions table does not exist yet, skipping trigger creation';
END
$$;
